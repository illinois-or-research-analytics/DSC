...
for nw_id in os.listdir(f"data/dsc/real/"):
    if nw_id not in nw_ids:
        print(f"Warning: {nw_id} is in data/dsc/real but not in data/networks_val.txt")
        nw_ids.append(nw_id)
...
for nw_id in nw_ids:
    if os.path.exists(f"data/dsc/real/{nw_id}"):
        for stage_id in os.listdir(f"data/dsc/real/{nw_id}"):
...
for nw_id in nw_ids:
    ...
    if os.path.exists(f"data/dsc/real/{nw_id}"):
        for stage_id in os.listdir(f"data/dsc/real/{nw_id}"):
            log_file = f"data/dsc/real/{nw_id}/{stage_id}/error.log"
...
df["stage_2"] = df["leiden-mod"] + df["rtrex"] + df["ikc-5"]
df["stage_3"] = df["merged"] + df["unweighted"]
df["stage_4"] = df["final"] + df["final+wcc"]
...
out_dir = Path("plots/sbm+wcc/time")

...
def compute_stats(input_network, input_clustering, output_folder, overwrite):
    ...
    # Determine the delimiter for the clustering file
    cluster_delimiter = detect_delimiter(input_clustering)

    # Generate the cluster mapping
    clustering_dict, cluster_mapping_dict = read_clustering(
        input_clustering, cluster_delimiter
    )
    cluster_mapping_dict_reversed = {v: k for k, v in cluster_mapping_dict.items()}
    ...
    with open(dir_path / OUTLIER_ORDERING_IDX_FILENAME, "w") as idx_f:
        outlier_ordering_idx_list = [
            [node_mapping_dict_reversed[node_iid]] for node_iid in outlier_order
        ]
    ...
    if "osub_degree" in stats_to_compute:
        osub_deg_distr = [o_subgraph.degree(u) for u in outlier_order]
    ...
    if "o_deg" in stats_to_compute:
        o_deg_distr = [graph.degree(u) for u in outlier_order]
    ...
    start_time = time.perf_counter()
    if (
        "n_disconnects" in stats_to_compute
        or "ratio_disconnected_clusters" in stats_to_compute
        or "n_wellconnected_clusters" in stats_to_compute
        or "ratio_wellconnected_clusters" in stats_to_compute
    ):
        n_disconnected_clusters = int((cluster_stats["connectivity"] < 1).sum())
        ratio_disconnected_clusters = (
            n_disconnected_clusters / n_clusters if n_clusters > 0 else 0
        )

        n_wellconnected_clusters = int(
    ...
    start_time = time.perf_counter()
    if (
        "participation_coeffs" in stats_to_compute
...
def prepare_logging(output_folder, is_overwrite):
    logging.basicConfig(
        filename=output_folder / "run.log",
...
def log_cpu_ram_usage(step_name):
    ...
    logging.info(
        f"Step: {step_name} | CPU Usage: {cpu_percent}% | RAM Usage: {
...
def read_clustering(filepath, delimiter="\t"):
    ...
    # # Remove cluster with a single node
    # cluster_df = cluster_df.groupby('cluster_name').filter(lambda x: len(x) > 1)
...
def get_outliers(graph, node_mapping, clustering_dict):
    ...
    # clustering_dict: {node_id: cluster_iid}
    # Ignore clusters that only have one node
    # First, count cluster sizes
    cluster_counts = Counter(clustering_dict.values())
    valid_clusters = {cid for cid, count in cluster_counts.items() if count > 1}
    clustered_nodes = [
        node_mapping[u]
        for u, cid in clustering_dict.items()
        if u in node_mapping and cid in valid_clusters
    ]
...
def compute_mixing_params(
    graph, clustering_dict, node_mapping_dict_reversed, node_order
):
    ...
    mus = [
        out_degree[i] / (out_degree[i] + in_degree[i]) if out_degree[i] > 0 else 0
        for i in node_order
    ]
    ...
    for v in graph.iterNodes():
        for neighbor in graph.iterNeighbors(v):
            neighbor_cluster = clustering_dict.get(
                node_mapping_dict_reversed[neighbor], -1
            )
...
def compute_participation_coeff_distr(
    graph, node_mapping_dict_reversed, clustering_dict, node_order, outlier_order
):
    ...
    participation_coeffs_distr = [participation_coeffs[v] for v in node_order]

    o_participation_coeffs_distr = [participation_coeffs[v] for v in outlier_order]

    return participation_coeffs_distr, o_participation_coeffs_distr


def load_clusters(
    filepath, cluster_iid2id, cluster_order, delimiter="\t"
    ...
    return [clusters[cluster_iid2id[cluster_iid]] for cluster_iid in cluster_order]
...
def compute_cluster_stats(
    ...
    mincuts = [viecut(cluster)[-1] for cluster in clusters]
    mincuts_normalized = [
        mincut / np.log10(ns[i]) if mincut > 0 else 0
        for i, mincut in enumerate(mincuts)
    ]

    conductances = [cluster.conductance(global_graph) for cluster in clusters]

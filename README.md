...
The code in [`src`](src) is a modification made by The-Anh Vu-Le of code provided by Elfarouk Harb in May 2025. Harb’s code (available at [this repo](https://github.com/FaroukY/CorporateEquivalence)) solves the dense subgraph problem using FISTA and Flow (Harb et al., NeurIPS 2025). The modifications allow iterative densest subgraph extraction for Flow. For FISTA, the modifications allow integer rounding, combined with iterative densest subgraph extraction for both integer rounding and fractional peeling.
